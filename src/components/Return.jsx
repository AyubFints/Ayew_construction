...
import { RotateCcw, ArrowLeft, BarChart3, User, PlusCircle, Trash2, CheckCircle, ClipboardList, CalendarDays, Filter, PackageMinus } from 'lucide-react';
...
const Return = ({ products, setProducts, returns, setReturns, setPage }) => {
  ...
  // --- BATAFSIL JADVAL UCHUN HOLATLAR ---
  const d = new Date();
  ...
  // --- BATAFSIL JADVAL HISOBO-KITOBI (Faqat Chiqimlar) ---
  ...
  // --- SAVATGA QO'SHISH (QAYTARISH UCHUN) ---
  ...
  const handleFinalReturn = () => {
    ...
    // 2. Kassaga bitta umumiy chiqim tarixi yozish
  ...
  return (
    <div className="fade-in">
      ...
      {/* QAYTARISH FORMASI (SAVAT TIZIMI) */}
          ...
          <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap', alignItems: 'flex-end', marginBottom: '15px' }}>
            ...
            {selectedProduct && (
              <div className="fade-in" style={{ flex: '1 1 100px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: '500' }}>Hajmi ({selectedProduct.unit})</label>
            ...
            <button type="submit" className="btn btn-danger" style={{ flex: '1 1 150px', height: '50px', display: 'flex', gap: '8px', justifyContent: 'center', backgroundColor: '#4b5563' }} disabled={products.length === 0}>
        ...
        {cart.length > 0 && (
          <div className="fade-in" style={{ marginTop: '30px', backgroundColor: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '20px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#e5e7eb', padding: '15px', borderRadius: '8px', marginBottom: '20px' }}>
              <span style={{ fontSize: '16px', fontWeight: 'bold', color: '#374151' }}>Umumiy Chiqim (Qaytariladigan pul):</span>
      ...
      {/* --- BATAFSIL JADVAL (SANA BO'YICHA FAQAT CHIQIMLAR) --- */}
      <div className="card" style={{ maxWidth: '100%', marginTop: '40px', borderTop: '4px solid #4b5563' }}>
        ...
        <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', backgroundColor: '#f9fafb', padding: '5px 15px', borderRadius: '8px', border: '1px solid #d1d5db' }}>
            ...
            <span style={{ fontWeight: '500', color: '#374151' }}>Sanani tanlang:</span>
        ...
        {/* Tanlangan kundagi chiqimlar ro'yxati */}
        {tableData.length === 0 ? (
          <p style={{ textAlign: 'center', color: '#6b7280', padding: '20px', backgroundColor: '#f9fafb', borderRadius: '8px', border: '1px dashed #d1d5db' }}>
            Tanlangan sana (<b>{detailDate}</b>) uchun hech qanday chiqim yo'q.
          </p>
        ) : (
          <div className="fade-in">
            {/* Jadval Sarlavhasi (Jami Summa) */}
            <div style={{ backgroundColor: '#f3f4f6', padding: '15px', borderRadius: '8px 8px 0 0', border: '1px solid #e5e7eb', borderBottom: 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontWeight: 'bold', color: '#4b5563', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <CalendarDays size={18} /> Sana: {detailDate}
              </span>
              <span style={{ fontWeight: 'bold', color: '#111827', fontSize: '16px' }}>
                Shu kundagi jami chiqim: -{tableTotalExpense.toLocaleString()} so'm
              </span>
            </div>
