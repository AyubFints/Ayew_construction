    ...
    <div className="fade-in">
      <style>
        {`
          ...
          .smart-logo {
            font-family: 'Montserrat', sans-serif; font-weight: 900; font-size: 46px; margin: 0;
            background: linear-gradient(90deg, #1e3a8a 0%, #10b981 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent;
            letter-spacing: -1.5px; text-transform: uppercase; display: flex; align-items: center; gap: 12px;
          }
          .smart-subtitle { margin: 5px 0 0 60px; font-size: 14px; color: #6b7280; font-weight: 600; letter-spacing: 3px; text-transform: uppercase; }
          .gradient-text { background: linear-gradient(90deg, #1e3a8a 0%, #10b981 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
          @media (max-width: 768px) {
            .dashboard-grid { display: grid !important; grid-template-columns: repeat(2, 1fr) !important; gap: 12px !important; }
            .menu-card { padding: 15px 10px !important; }
            .menu-icon svg { width: 32px !important; height: 32px !important; }
            .menu-title { font-size: 13px !important; }
            .smart-logo { font-size: 32px; }
            .smart-subtitle { margin-left: 45px; font-size: 11px; letter-spacing: 2px; }
            .header-top { flex-direction: column; gap: 20px; }
            .logout-btn { width: 100% !important; justify-content: center; }
          }
        `}
      </style>
      ...
      <div className="header-top" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '40px' }}>
        <div>
          <div style={{ marginBottom: '25px', paddingBottom: '20px', borderBottom: '2px dashed #e5e7eb' }}>
            <h1 className="smart-logo"><Store size={46} color="#1e3a8a" style={{ filter: 'drop-shadow(0px 4px 6px rgba(0,0,0,0.1))' }} /> SMART DO'KON</h1>
            <p className="smart-subtitle">Avtomatlashtirilgan Tizim</p>
            <p className="smart-subtitle">Do'koningiz endi Smartfonda!</p>
          </div>
      ...
      <div className="dashboard-grid" style={{ marginBottom: '40px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
        ...
        <div className="fade-in card" style={{ padding: '25px', backgroundColor: '#ffffff', borderRadius: '16px', border: '1px solid #e5e7eb', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.05)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '15px' }}>
            ...
            <select 
              ...
              style={{ padding: '8px 12px', borderRadius: '8px', border: '1px solid #cbd5e1', backgroundColor: '#f8fafc', color: '#1e3a8a', fontWeight: 'bold', outline: 'none', cursor: 'pointer' }}
            >
